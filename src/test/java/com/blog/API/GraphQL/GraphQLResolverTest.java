...
class GraphQLResolverTest {
    ...
    class Login {
        ...
        void login_Success() {
            when(authService.login("user", "pass")).thenReturn(Map.of("accessToken", "acc", "refreshToken", "ref", "type", "Bearer"));
        ...
        void login_Failure_PropagatesException() {
            when(authService.login("user", "wrong")).thenThrow(new RuntimeException("Invalid credentials"));
