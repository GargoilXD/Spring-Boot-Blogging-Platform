...
public class RestPostController {
    ...
    public CompletableFuture<ResponseEntity<SuccessResponse<ResponsePostDTO>>> createPost(@Valid @RequestBody CreatePostDTO createDTO) {
        String username = SecurityContextHolder.getContext().getAuthentication().getName();
        return postService.save(createDTO, username).thenApply(dto -> {
            ResponsePostDTO response = new ResponsePostDTO(dto);
    ...
    @PreAuthorize("hasAnyRole('AUTHOR', 'ADMIN')")
    public CompletableFuture<ResponseEntity<SuccessResponse<ResponsePostDTO>>> updatePost(
            @Parameter(description = "ID of the post to update", required = true, example = "1")
            @PathVariable Integer id,
            @Valid @RequestBody UpdatePostDTO updateDTO
    ) {
        String username = SecurityContextHolder.getContext().getAuthentication().getName();
        return postService.update(id, updateDTO, username).thenApply(dto -> {
            ResponsePostDTO response = new ResponsePostDTO(dto);
